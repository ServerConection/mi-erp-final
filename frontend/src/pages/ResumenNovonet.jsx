...
// ── Paleta NOVONET — pasteles azul/verde ──────────────────────────────────────
const P = {
  blue:   "#60a5fa",
  green:  "#34d399",
  amber:  "#fbbf24",
...
const COLORES_REG = {
  ...
  "NO REQUIERE REGULARIZAR": P.blue,
  "SIN DATO":                P.slate,
};
...
export default function ResumenNovonet() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/analista/novonet?desde=${desde}&hasta=${hasta}`,
  ...
  // Scatter data — eje X: total, eje Y: % activos
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 space-y-6">
      ...
      {data && !loading && (<>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <KpiCard icon="👤" label="Asesores"        value={data.asesores?.length || 0} color="text-purple-500"
        ...
        <Card icon="📈" title="Tendencia Diaria — Ingresos Jotform">
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={data.tendencia} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="total"   name="Total JOT"  stroke={P.blue}  fill="url(#gTotal)"   strokeWidth={2} dot={false} />
              <Area type="monotone" dataKey="activos" name="Activos"    stroke={P.green} fill="url(#gActivos)" strokeWidth={2} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>

        {/* ── Calidad + Regularización ── */}
        ...
        <Card icon="🎯" title="Matriz de Rendimiento — Volumen vs. % Activos por Asesor">
          ...
          <div className="grid grid-cols-2 gap-1 text-[9px] text-slate-400 mb-2 text-center">
            ...
            <div className="bg-blue-50 rounded-lg py-1 border border-blue-100">⬇️➡️ Alto volumen, mejorar calidad</div>
          </div>
          <ResponsiveContainer width="100%" height={300}>
            <ScatterChart margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
              ...
              <Scatter name="Asesores" data={scatterData.points}
                fill={P.blue} fillOpacity={0.75} stroke={P.blue} strokeWidth={1} />
            </ScatterChart>
          </ResponsiveContainer>
        </Card>
        ...
        {/* ── Ranking asesores — bar horizontal ── */}
        <Card icon="🏆" title="Ranking de Asesores — Top 15 por Volumen">
          <ResponsiveContainer width="100%" height={Math.max(250, data.asesores.length * 26)}>
            <BarChart data={data.asesores} layout="vertical"
              ...
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: 10, color: "#64748b" }} />
              <Bar dataKey="total"   name="Total"   fill={P.blue}  radius={[0, 4, 4, 0]}>
