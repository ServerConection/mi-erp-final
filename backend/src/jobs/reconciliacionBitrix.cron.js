...
const correr = async ({ desde: desdeForzado } = {}) => {
  const desde = desdeForzado || new Date(Date.now() - DIAS_VENTANA * 864e5).toISOString().slice(0, 10);
  let lock;
  try {
...
function initReconciliacionBitrix() {
  ...
  // ENCENDIDO POR DEFECTO (2026-09-22): el receptor de eventos
  // (bitrixEvento.routes.js) nunca se montó en app.js, así que NADA
  // corregía el origen/etapa cuando cambiaban en Bitrix sin disparar el
  // webhook de etapas. Se apaga con RECONCILIACION_BITRIX=off.
  if ((process.env.RECONCILIACION_BITRIX || 'on').toLowerCase() !== 'on') {
  ...
  // Primera corrida a los 30 s: desde el día 1 del mes en curso, para dejar
  // cuadrado el mes que se reporta en Redes aunque el deal no se haya tocado
  // en los últimos días.
  const inicioMes = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Guayaquil' })
    .format(new Date()).slice(0, 8) + '01';
  setTimeout(() => correr({ desde: inicioMes }), 30000);
}
