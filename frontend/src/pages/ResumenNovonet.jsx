...
const COLORES_ESTADO = {
  ...
  "NO REQUIERE REGULARIZAR":   "#3b82f6",
  "SIN DATO":                  "#64748b",
};
const COLORES_PIE = ["#3b82f6","#10b981","#f59e0b","#ef4444","#8b5cf6","#06b6d4","#f97316","#64748b"];
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API}/api/analista/novonet?desde=${desde}&hasta=${hasta}`,
    ...
    <div className="min-h-screen bg-slate-900 text-white p-6 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Análisis de calidad de ventas — Jotform</p>
        </div>
        {/* Filtro de fechas */}
        <div className="flex items-center gap-2 bg-slate-800 rounded-2xl p-3 border border-slate-700">
          ...
          <button
            onClick={fetchData}
            className="mt-4 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold px-4 py-2 rounded-xl transition-colors"
          >
            Aplicar
          </button>
        </div>
      ...
      {data && !loading && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <KpiCard label="Regularizados" value={kpis.regularizados} color="text-blue-400"
              sub={`${kpis.pctRegularizados}% del total`} />
            <KpiCard label="Asesores activos" value={data.asesores?.length || 0} color="text-purple-400" />
          </div>
          ...
          <div className="bg-slate-800 rounded-2xl p-5 border border-slate-700">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={data.tendencia} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="total" name="Ingresos JOT" stroke="#3b82f6" strokeWidth={2} dot={false} />
            ...
            <div className="bg-slate-800 rounded-2xl p-5 border border-slate-700">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  ...
                  <Tooltip formatter={(v, n) => [v, n]} contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 12 }} />
                  <Legend wrapperStyle={{ fontSize: 11, color: "#94a3b8" }} />
                </PieChart>
              </ResponsiveContainer>
            </div>

            {/* Estado Regularización — Pie */}
            <div className="bg-slate-800 rounded-2xl p-5 border border-slate-700">
