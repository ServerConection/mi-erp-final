...
// ─────────────────────────────────────────────────────────────────────────────
// METAS MENSUALES — ajusta según Velsa
// ─────────────────────────────────────────────────────────────────────────────
const METAS = {
  gestionables: 200,
  ingresos_crm: 80,
...
// ─────────────────────────────────────────────────────────────────────────────
// PILL COLOR — estados Netlife Velsa
// ─────────────────────────────────────────────────────────────────────────────
const PILL_CONFIG = {
  ...
  "FUERA DE COBERTURA":    { bg: "#f3f4f6", color: "#6b7280", border: "#d1d5db" },
  "DESISTE DEL SERVICIO":  { bg: "#fef9c3", color: "#92400e", border: "#fcd34d" },
...
function ClienteModal({ cliente, onClose }) {
  ...
  const LABELS = {
    FECHA_CREACION_JOT:   "Fecha registro Jot",
    ID_CRM:               "ID CRM",
    ESTADO_NETLIFE:       "Estado Netlife",
    FECHA_ACTIVACION:     "Fecha activación",
    NOVEDADES_ATC:        "Cláusulas / Novedades",
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{
              ...
              background: "#fdf4ff", border: "2px solid #a855f7",
              display: "flex", alignItems: "center", justifyContent: "center",
        ...
        <div style={{ padding: "16px 20px 20px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {Object.entries(LABELS).map(([key, label]) => {
              ...
              const fullWidth = ["NOVEDADES_ATC", "MOTIVO_REGULARIZAR", "TERCERA_EDAD"].includes(key);
              return (
                <div key={key} style={{
...
function RankBadge({ pos }) {
  const configs = [
    { bg: "#fdf4ff", color: "#7e22ce", border: "#d8b4fe", icon: "🥇" },
    { bg: "#f5f3ff", color: "#5b21b6", border: "#c4b5fd", icon: "🥈" },
    { bg: "#faf5ff", color: "#6d28d9", border: "#ddd6fe", icon: "🥉" },
  ];
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow = "0 6px 24px #a855f715";
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            background: "#fdf4ff", border: "2px solid #a855f7",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 12, fontWeight: 800, color: "#9333ea",
          }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 1, background: "#f1f5f9" }}>
        {[
          { label: "Leads Gest.", val: row.gestionables,    color: "#a855f7" },
          { label: "Ingr. CRM",   val: row.ventas_crm,       color: "#6366f1" },
      ...
      {/* BARRAS + ETAPAS en fila horizontal */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, padding: "12px 16px 14px" }}>
        ...
        {etapas.length > 0 && (
          <div style={{
            ...
            justifyContent: "flex-start", paddingTop: 2, minWidth: 150,
          }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: "#94a3b8",
              textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 4 }}>
              Etapas Jot
...
// ─────────────────────────────────────────────────────────────────────────────
export default function VistaAsesorVelsa() {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-5 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <span className="bg-purple-500 text-white px-2 py-1 rounded text-[11px] font-black uppercase tracking-wider">
            VELSA
          </span>
          <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded text-[11px] font-black uppercase tracking-wider">
            ASESORES
          </span>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-6 shadow-sm">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 items-end">
          <div className="lg:col-span-2 flex flex-col gap-2">
            ...
            <div className="flex bg-slate-50 border border-slate-200 rounded-xl p-1.5 focus-within:border-purple-400 transition-colors">
              <input type="date"
          ...
          {/* Asesor dropdown — poblado desde el backend */}
          ...
          {/* Estado Netlife — valores reales de Velsa */}
          <div className="flex flex-col gap-2">
            ...
            <select className={inputCls} value={filtros.estadoNetlife}
              ...
              <option value="DESISTE DEL SERVICIO">DESISTE DEL SERVICIO</option>
            </select>
          </div>

          {/* Regularización — campo t2_regularizado en Velsa */}
          <div className="flex flex-col gap-2">
          ...
          <button onClick={() => fetchData()}
            className="bg-purple-500 hover:bg-purple-400 text-white h-[42px] rounded-xl text-[10px] font-black uppercase tracking-wider shadow-md shadow-purple-200 transition-all active:scale-95">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        ...
        <StripCard label="Ingresos CRM"       value={totales.ingresos_crm}   color="#6366f1"
          meta={METAS.ingresos_crm * (asesoresEnriquecidos.length || 1)} />
        <StripCard label="Ingresos Jotform"   value={totales.ingresos_jot}   color="#10b981"
          meta={METAS.ingresos_jot * (asesoresEnriquecidos.length || 1)} />
        <StripCard label="Activas mes"        value={totales.activas_mes}    color="#f59e0b"
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 flex-wrap">
            ...
            {filtros.etapaJotform && (
              <button
                ...
                className="ml-2 bg-purple-50 text-purple-600 hover:bg-red-50 hover:text-red-500 border border-purple-200 px-2 py-0.5 rounded-full font-black transition-colors text-[8px]"
              >
                ✕ {filtros.etapaJotform}
              </button>
            )}
          ...
          <div className="flex flex-wrap gap-2">
            {estadosNetlife.map(({ estado, total }) => {
              ...
              return (
                <button key={estado}
                  onClick={() => {
                    const nuevo = activo ? "" : estado;
                    const f = { ...filtros, etapaJotform: nuevo };
                    setFiltros(f); fetchData(f);
                  }}
                  style={{
      ...
      {/* ── RANKING ASESORES — 1 por fila, scroll vertical ── */}
      <div className="mb-8">
