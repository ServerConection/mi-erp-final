 ...
 *
 * Es el MISMO endpoint para las 53 etapas — lo único que cambia entre una
 * automatización y otra es el valor fijo de "etapa" en la URL (tú lo escribes
 * una vez al configurar cada automatización, no es un placeholder de Bitrix).
 * Ver GUIA_WEBHOOKS_BITRIX_ETAPAS.md en la raíz del repo para las 53 URLs.
 *
 * Trazabilidad:
 *   - bitrix_webhook_leads            → 1 fila por lead (bitrix_id), UPSERT:
 *     la etapa más reciente SIEMPRE reemplaza a la anterior en esta tabla.
...
// Normaliza el valor de "etapa" para que coincida con los slugs del catálogo
// aunque alguien pegue el nombre con espacios/acentos/mayúsculas por error.
const slugifyEtapa = (valor = '') =>
  String(valor)
...
const recibirLead = async (req, res) => {
  try {
    ...
    const etapa = slugifyEtapa(req.query.etapa || '');
    ...
    const columnas = ['bitrix_id', 'etapa', ...CAMPOS_BITRIX, 'raw_query'];
    ...
    if (!id) {
      await pool.query(
        ...
        [null, etapa, ...valores, JSON.stringify(req.query)]
      );
      console.warn('[bitrixWebhook] Webhook sin ID recibido — solo se guardó en historial. etapa:', etapa);
      return res.status(200).send('OK (sin ID, no se pudo actualizar estado actual)');
    }

    const paramsUpsert = [id, etapa, ...valores, JSON.stringify(req.query)];
    const setClause = [...CAMPOS_BITRIX, 'raw_query']
      .map(c => `${c} = EXCLUDED.${c}`)
      .concat(['etapa = EXCLUDED.etapa', 'updated_at = NOW()'])
      .join(', ');

    await pool.transaction(async (client) => {
      // 1) Estado ACTUAL — la etapa nueva reemplaza a la anterior para este bitrix_id
      await client.query(
        `INSERT INTO bitrix_webhook_leads (${columnas.join(',')})
         VALUES (${placeholders})
         ON CONFLICT (bitrix_id) DO UPDATE SET ${setClause}`,
        paramsUpsert
      );
...
const listarLeads = async (req, res) => {
  try {
    const { limit = 50, offset = 0, etapa = null } = req.query;
    const params = [];
    let where = '';
    if (etapa) { params.push(slugifyEtapa(etapa)); where = `WHERE etapa = $${params.length}`; }
...
const historialLead = async (req, res) => {
  try {
    ...
    return res.json({ success: true, bitrix_id, recorrido: r.rows, count: r.rows.length });
  } catch (err) {
