...
// Solo visible para perfil ADMINISTRADOR (filtro adicional en el componente).
// Interfaz futurista dark blue tipo la imagen de referencia.
// =============================================================================
...
// Helper para sonidos generados con WebAudio (sin archivos externos)
const audioCtx = (() => {
...
const playGoalSound = (volume = 0.4) => {
  ...
  // estadio: subir tres notas y un "ROAR" con ruido
...
// =============================================================================
// OVERLAY "GOOOOL!" — pantalla completa
// =============================================================================
...
const Styles = () => (
  <style>{`
    ...
    @keyframes pulseGreen {
      ...
      50%      { box-shadow: 0 0 0 4px rgba(16,185,129,.4); }
    }
    .mundi-leader-row {
    ...
    .mundi-table-row:hover { background: rgba(59,130,246,.08); }
    .mundi-btn {
...
export default function Mundialito() {
  ...
  // Carga datos del torneo seleccionado
  ...
  useEffect(() => { if (isAdmin) cargarTorneos(); }, [empresa]);
  useEffect(() => { cargarDatosTorneo(torneo); }, [torneo]);

  // Socket.IO live: escucha goles y sorteos
  useEffect(() => {
    if (!isAdmin) return;
  ...
  const registrarGolManual = async (asesor_id, tipo, cantidad = 1) => {
    ...
    // la actualizacion llega via Socket.IO
  };
  ...
  if (!isAdmin) {
    return (
      <div style={{ padding: 40, textAlign: "center", color: "#cbd5e1" }}>
        <h2 style={{ fontWeight: 900 }}>Acceso restringido</h2>
        <p>El modulo Mundialito esta reservado para perfil ADMINISTRADOR.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ position: "absolute", inset: 0, opacity: .08, fontSize: 48 }}>
          <div className="mundi-ticker" style={{ whiteSpace: "nowrap" }}>⚽⚽⚽⚽⚽⚽⚽⚽⚽⚽⚽⚽⚽⚽</div>
        </div>
              ...
              <tbody>
                {posiciones.map((p, idx) => (
                  <tr key={p.participante_id}
                    ...
                    <td style={{ ...td, fontSize: 16, fontWeight: 900, color: "#fff" }}>
                      ⚽ {p.goles_totales}
                    </td>
                    <td style={{ ...td, fontSize: 18, fontWeight: 900, color: "#fbbf24" }}>
                      {p.puntos_total}
                    </td>
                  </tr>
                ))}
      ...
      {tab === "partidos" && torneo && (
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 12 }}>
            ...
            {partidos.length === 0 && (
              <div style={{ color: "#94a3b8", textAlign: "center", padding: 32 }}>No hay partidos generados. Usa el boton "Generar Partidos del Dia".</div>
            )}
          </div>
        </div>
      )}
