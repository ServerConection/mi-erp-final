/**
 * MIGRACIÓN BITRIX24 — ejecutar UNA SOLA VEZ
 * Desde la carpeta backend: node migrate.js
 */
...
CREATE INDEX IF NOT EXISTS idx_bx_fecha    ON bitrix_deals(fecha_creacion);
CREATE INDEX IF NOT EXISTS idx_bx_cat      ON bitrix_deals(category_id);
CREATE INDEX IF NOT EXISTS idx_bx_stage    ON bitrix_deals(stage_id);
CREATE INDEX IF NOT EXISTS idx_bx_asesor   ON bitrix_deals(asesor_id);
CREATE INDEX IF NOT EXISTS idx_bx_ganado   ON bitrix_deals(ganado);
...
(async () => {
  ...
  try {
    ...
    const r = await client.query(`
      ...
      WHERE schemaname='public' AND tablename LIKE 'bitrix%'
      ORDER BY tablename
    `);
