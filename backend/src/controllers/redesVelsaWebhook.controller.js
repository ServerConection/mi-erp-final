...
async function getMonitoreoRedesVelsa(req, res) {
  try {
    ...
    const inv=await pool.query(`SELECT canal_publicidad,SUM(monto_usd) inversion FROM velsa_inversion_redes WHERE fecha BETWEEN $1 AND $2 GROUP BY 1`,[desde,hasta]);
...
async function getTendenciaDiaria(req,res){
  try{await asegurarInversionReciente();const {desde,hasta}=fechas(req.query),canales=seleccion(req.query);const rows=(await consultarDiario(desde,hasta,canales)).rows;
    ...
    const inv=await pool.query(`SELECT fecha,SUM(monto_usd) inversion FROM velsa_inversion_redes WHERE fecha BETWEEN $1 AND $2 GROUP BY 1`,[desde,hasta]);for(const r of inv.rows){const f=new Date(r.fecha).toISOString().slice(0,10);if(!map[f])map[f]={fecha:f,n_leads:0,gestionables:0,atc:0,venta_subida:0,descartados:0};map[f].inversion=Number(r.inversion||0);}
