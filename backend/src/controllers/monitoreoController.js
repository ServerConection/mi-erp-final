...
const getReporteData = async (req, res) => {
  try {
    ...
    // ── FIX: leer solo ?canales= — si viene vacío = todos ────────────────────
    ...
    // Si canalesRaw vacío → origenes=[] y canalesPublicidad=[] → sin filtro = todos
    ...
    // ── BLOQUE 1: Solo inversión desde la vista (MAX por canal×día) ───────────
    ...
    // ── BLOQUE 2: Leads + Etapas desde mestra_bitrix (fuente real) ────────────
    ...
    // ── BLOQUE 3: Denominadores JOT reales (activos, backlog, etc.) ───────────
    // JOIN mestra_bitrix → vw_jotform_submissions_parsed para obtener
    // los valores correctos desde JOT agrupados por día de creación del lead
    const jotDenomsRes = await pool.query(`
      SELECT
        EXTRACT(DAY FROM b.b_creado_el_fecha::date)::int AS dia,
        COUNT(j.submission_id) FILTER (
          WHERE j.netlife_estatus ~~* '%ACTIVO%' AND j.fecha_activacion_date IS NOT NULL
        ) AS activos_mes,
        COUNT(j.submission_id) FILTER (
          WHERE j.netlife_estatus ~~* '%ACTIVO%'
        ) AS activo_backlog,
        COUNT(j.submission_id) FILTER (
        ...
        COUNT(j.submission_id) FILTER (
          WHERE j.netlife_estatus ~~* '%PREPLANEADO%' OR j.netlife_estatus ~~* '%REPLANIFICADO%'
        ) AS preplaneados,
        COUNT(j.submission_id) FILTER (
          WHERE j.netlife_estatus ~~* '%ASIGNADO%'
        ) AS asignados,
        COUNT(j.submission_id) FILTER (
          WHERE j.netlife_estatus ~~* '%PRESERVICIO%'
        ) AS preservicio,
        COUNT(j.submission_id) FILTER (
          WHERE j.netlife_estatus ~~* '%DESISTE%'
        ) AS desiste_servicio_jot,
        COUNT(j.submission_id) FILTER (
          WHERE j.regularizado ~~* '%REGULARIZADO%'
            AND j.regularizado !~~* '%NO REQUIERE%'
            AND j.regularizado !~~* '%POR REGULARIZAR%'
        ) AS regularizados,
        COUNT(j.submission_id) FILTER (
          WHERE j.regularizado ~~* '%POR REGULARIZAR%'
        ) AS por_regularizar,
    ...
    // ── Combinar inversión + denominadores en un solo array para el frontend ──
    const invMap = {};
    inversionRes.rows.forEach(r => {
      invMap[r.dia] = { dia: Number(r.dia), inversion_usd: Number(r.inversion_usd || 0) };
    });
    // Agregar leads y venta_subida desde mestra_bitrix
    ...
    // Agregar denominadores JOT reales
    ...
    // ── BLOQUE 4: Estatus ventas JOT por día (para tabla JOT) ─────────────────
    ...
    // ── BLOQUE 5: Forma de pago desde JOT ─────────────────────────────────────
    ...
    // ── BLOQUE 6: Ciclo de venta desde JOT ────────────────────────────────────
    ...
    res.json({
      ...
      inversion:   inversionFinal,   // inversión + denominadores JOT reales combinados
