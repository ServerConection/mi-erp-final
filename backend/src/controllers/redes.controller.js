...
// FIX DEFINITIVO:
// - Filtra canal_inversion NOT IN ('MAL INGRESO','SIN MAPEO') para excluir no mapeados
// - activos_mes viene de la vista ya corregida (misma fecha mes/año)
// - Se devuelve canal_inversion como canal para que el frontend no re-mapee
// ─────────────────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    const detalleResult = await pool.query(`
      SELECT
        ...
        -- Inversión: sumar MAX por canal×día (ya deduplicada en la vista)
...
// FIX: los indicadores (CPL, CPA, etc.) ahora se calculan correctamente
// usando los datos de Bitrix (leads, ventas) + inversión de mv_monitoreo_publicidad
// ─────────────────────────────────────────────────────────────────────────────
const getMonitoreoMetas = async (req, res) => {
  try {
    ...
    // Inversión real del período por canal desde mv_monitoreo_publicidad
    ...
    const canales = Object.values(grupoMap)
      ...
      .map(c => {
        ...
        return {
          ...
          // FIX: calcular indicadores aquí para que el frontend los muestre
...
// ─────────────────────────────────────────────────────────────────────────────
// 7. REPORTE DATA
// FIX: inversión usa SUM(inversion_usd) deduplicada por canal×día
//      activos_mes viene de jotDenomsRes con filtro fecha_activacion en el rango
// ─────────────────────────────────────────────────────────────────────────────
const getReporteData = async (req, res) => {
  try {
    ...
    // ── Inversión diaria — deduplicada por canal×día ──────────────────────────
    // FIX: SUM(MAX(inversion_usd)) agrupa correctamente, evitando multiplicación
    ...
    const jotDenomsRes = await pool.query(`
      SELECT
        ...
        -- FIX activos_mes: fecha_activacion dentro del rango solicitado
