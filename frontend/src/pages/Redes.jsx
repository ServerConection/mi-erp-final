...
// FIX: getCanal también acepta canal_inversion directo (ya agrupado)
const getCanal = (o) => {
  if (!o) return "SIN MAPEO";
  // Si ya es un canal agrupado conocido, devolverlo directo
  if (CANALES[o]) return o;
  // Si es un origen raw, mapearlo
...
// ─────────────────────────────────────────────────────────────────────────────
// AGREGADORES
// FIX DEFINITIVO: el backend ya devuelve canal_inversion agrupado.
// agregarPorCanal usa canal_inversion directamente sin re-mapear.
// ─────────────────────────────────────────────────────────────────────────────
function agregarPorCanalDia(filas) {
  const map = {};
  filas.forEach((row) => {
    // canal_inversion ya viene agrupado del backend (ej: "ARTS GOOGLE")
    ...
    // Inversión ya viene deduplicada del backend por canal×día
...
function agregarPorCanal(filas) {
  // FIX DEFINITIVO ACTIVOS:
  // El backend agrupa por canal_inversion y suma activos_mes correctamente.
  // Aquí solo acumulamos lo que viene del backend sin re-mapear nada.
  const map = {}, invC = {};
  filas.forEach((row) => {
    const canal = row.canal_inversion || getCanal(row.canal_publicidad);
    if (!esPublicidad(canal)) return; // excluir MAL INGRESO y SIN MAPEO
    ...
    a.venta_subida_bitrix += n(row.venta_subida_bitrix);
    a.ingreso_jot         += n(row.ingreso_jot);
    a.activos_mes         += n(row.activos_mes); // FIX: suma directo, ya viene correcto del backend
...
function TabMonitoreoGeneral({ data, loading }) {
  ...
  const porCanal = agregarPorCanal(rawFilas); // ya filtra esPublicidad dentro
        ...
        <Card>
          ...
          <div className="overflow-auto max-h-72">
            {vistaAtc==="resumen" ? (
              ...
              <table className="text-[9px] font-mono border-collapse w-full whitespace-nowrap">
                <thead className="sticky top-0 border-b" style={{ background: C.light, borderColor: C.border }}>
                  <tr>{["FECHA","MOTIVO","CANT."].map(h=><th key={h} className="px-3 py-2 border-r text-center font-black uppercase" style={{ color: C.muted, borderColor: C.border }}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {(atc?.data||[]).map((row,i)=>(
                    <tr key={i} className="border-b hover:bg-slate-50" style={{ borderColor: C.border }}>
  ...
  return (
    <div className="space-y-6">
      ...
      {!loadingData && canalesData.length>0 && canalesData.map((canal)=>{
        const cNombre=canal.canal, cfg=getCfg(cNombre), filas=buildFilas(canal, metas);
